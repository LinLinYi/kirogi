...
UClusterJoinPage::UClusterJoinPage(QWidget* parent, UClusterInfo& info)
    :BasePage(parent)
    , btnTitleInfo(NULL)
    , btnDescribe(NULL)
    , curClusterInfo(info)
...
void UClusterJoinPage::initializeWidget()
{
    //提示信息页面操作;
    ...
    if(this->hasJoinedCluster()){
        btnJoinDevCluster->setText(tr("返回联系人页"));
...
bool UClusterJoinPage::hasJoinedCluster()
{
    quint64 clusID = curClusterInfo.getClusterID();
    return UClusterInfo::isExistClusterInfo(clusID);
}

void UClusterJoinPage::fetchData(InputPacket& inpack)
{
...
void UClusterJoinPage::handleHintInfo(QString& text)
{
...
void UClusterJoinPage::handleNetError()
{
...
void UClusterJoinPage::handleJoinDevCluster(InputPacket &inpack)
{
...
void UClusterJoinPage::setClusterInfo(UClusterInfo& info)
{
...
void UClusterJoinPage::on_btnJoinDevCluster_clicked()
{
    quint64 clusID = curClusterInfo.getClusterID();
    ...
    emit readyRead(CommandCode::UCLUSTER_APPLY_REQ, data, dataLen);
...
void UClusterJoinPage::on_backClusterManagementPage_clicked()
{
    quint64 clusID = curClusterInfo.getClusterID();
    AssemblyData assemblyData;
    assemblyData.append(clusID);
    char* data = assemblyData.getAssemblyData();
    quint16 dataLen = assemblyData.getAssemblyDataLen();
    emit readyRead(CommandCode::BACK_CONTACT_PAGE, data, dataLen);
}

void UClusterJoinPage::on_hintMsgPage(bool isJump)
{
    if(!isJump){
        hintMsgPage->hide();
        return;
...
void UClusterJoinPage::on_btnPrePage_clicked()
{
    this->hide();
    this->deleteLater();
}

UClusterJoinPage::~UClusterJoinPage()
{
