...
UClusterNotifyInfo::UClusterNotifyInfo(QObject* parent) : QObject(parent){}

void UClusterNotifyInfo::unpack(char *data, MiCo::ClusterNotify notify)
{
    if(data == NULL) return;
...
UClusterNotifyInfo::UClusterNotifyInfo(const UClusterNotifyInfo& info)
{
...
UClusterNotifyInfo& UClusterNotifyInfo::operator =(const UClusterNotifyInfo& info)
{
    m_msgType = info.m_msgType;
    m_userID = info.m_userID;
    m_userName = info.m_userName;
...
void UClusterNotifyInfo::updateInfoStatus(MiCo::MsgStatus status)
{
    m_msgStatus = status;
    //UClusterNotifyDataSource::updateCheckStatus(m_msgType, m_userID, m_clusterID, m_msgStatus);
}

void UClusterNotifyInfo::saveToLocal()
{
...
QString UClusterNotifyInfo::getCurrentTime()
{
...
UClusterNotifyInfo::~UClusterNotifyInfo()
{
