...
void MD5::calc(const char *inStr, size_t inStrLen, char* outStr)
{
    update(inStr, inStrLen);

    digest();
    memcpy(outStr, _digest, 16);
    reset();
}
...
void MD5::final() {
    ...
    memcpy((void *)oldState, _state, 16);
    memcpy((void*)oldCount, _count, 8);
    ...
    memcpy((void*)_state, oldState, 16);
    memcpy((void*)_count, oldCount, 8);
}
